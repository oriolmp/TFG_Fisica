...
class LayeredNetworkGraph(object):
    ...
    def __init__(self, graphs, node_labels=None, node_color=None, plane_labels=None, plane_colors=None, layout=nx.spring_layout, ax=None):
    ...
    def draw_edges(self, edges, *args, **kwargs):
        ...
        line_collection = Line3DCollection(segments, lw=0.7, *args, **kwargs)
    ...
    def draw_plane(self, z, plane_label="", plane_color="gray", *args, **kwargs):
        (xmin, xmax), (ymin, ymax) = self.get_extent(pad=0.1)
        u = np.linspace(xmin, xmax, 10)
        v = np.linspace(ymin, ymax, 10)
        U, V = np.meshgrid(u ,v)
        W = z * np.ones_like(U)
        self.ax.plot_surface(U, V, W, color=plane_color, *args, **kwargs)

        self.ax.text(xmax, ymax, z-0.15, plane_label)
    ...
    def draw(self):
        ...
        for z in range(self.total_layers):
            self.draw_plane(z, plane_label=self.plane_labels[z], plane_color=self.plane_colors[z], alpha=0.2, zorder=1)
            self.draw_nodes([node for node in self.nodes if node[1]==z], color=self.node_color, s=100, zorder=3)
...
if __name__ == '__main__':
    ...
    LayeredNetworkGraph([i, i], node_labels=node_labels, ax=ax, layout=nx.spring_layout)
    ax.set_axis_off()
    plt.show()
