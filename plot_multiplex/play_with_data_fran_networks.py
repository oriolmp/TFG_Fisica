...
df_C_yo = df_C.yo_final[:, df_C.yo_final.count() > N_PA].dropna(how='any')
df_C_otro = df_C.otro_final[:, df_C.otro_final.count() > N_PA].dropna(how='any')
...
fig = plt.figure(figsize=(20,3), num=super_title_cat[sociodem_entry] )#figsize=plt.figaspect(0.6))
...
for axi, story in enumerate(["Compartir", "Experiencia_aprenentatge", "Ingresada"]):#'capacitation_Teatre_Amigues', 'Obrir_camí', 'Experiencia_aprenentatge', 'Gossos', 'Sanglotant']):
    ...
    if layer_tp=="uniplex":
        ax2.set_title("Has tingut la mateixa vivència? >>> Sí.")
...
ax6.set_ylabel("")
import pickle
pickle.dump(fig, open('FigureObject_'+sociodem_entry+'_'+layer_tp+'.pickle', 'wb')) 
#plt.show()
